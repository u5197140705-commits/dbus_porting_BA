...
static bool dbal_is_con_transmit_index(uint8_t message_index);
static bool dbal_is_temp_con_message(uint8_t con_msg_type);
static enum DBAL_MessageType dbal_convert_uint8_to_dbal_type(uint8_t value);
static enum DBAL_MessageType dbal_get_response_dbal_type(enum DBAL_MessageType req_dbal_type);
static bool dbal_is_msg_of_dbal_type_to_repeat(enum DBAL_MessageType dbal_type);
static bool dbal_is_msg_of_ack_dbal_type(enum DBAL_MessageType dbal_type);
static void dbal_reset_events_to_ack(void);
static bool dbal_is_event_already_acknowledged(uint16_t service_id, uint16_t command_id);
static void dbal_set_code_section_flag(struct dbal_instance* const inst, uint8_t mask);
static bool dbal_check_task_code_section_flag(const struct dbal_instance* const inst);
static void dbal_execute_set_non_task_code_sections(struct dbal_instance* const inst);
static void dbal_clear_io_transmit_buffer(struct dbal_instance* const inst);
static void dbal_clear_io_tx_repeat_buffer(struct dbal_instance* const inst);
static void dbal_clear_both_io_tx_buffers(struct dbal_instance* const inst);
static bool dbal_is_io_transmit_buffer_empty(const struct dbal_instance* const inst);
static bool dbal_is_io_tx_repeat_buffer_empty(const struct dbal_instance* const inst);
static bool dbal_is_disable_pending(const struct dbal_instance* const inst);
static void dbal_clear_retry_counters(struct dbal_instance* const inst);
static void dbal_clear_msgs_to_repeat(struct dbal_instance* const inst);
static void dbal_check_and_fire_msgs_to_repeat_when_clearing(const struct dbal_instance* const inst);
static void dbal_close_gap_between_msgs_to_repeat(struct dbal_instance* const inst, uint8_t start_index);
static void dbal_organize_msgs_to_repeat(struct dbal_instance* const inst);
static void dbal_handle_con_msg_tx_fail(struct dbal_instance* const inst);
static void dbal_handle_req_resp_msg_tx_fail(struct dbal_instance* const inst);
static void dbal_msg_timer_action(struct dbal_instance* const inst);
static void dbal_prepare_generic_frame_header(uint8_t* const buffer);
static void dbal_store_payload_to_transmit_buffer(uint8_t* const buffer, uint8_t from, const uint8_t* const bytes, uint8_t data_len);
static bool dbal_is_msg_to_repeat_saved(const struct dbal_instance* const inst, enum DBAL_MessageType dbal_type, uint16_t service_id, uint16_t command_id, const uint8_t* const bytes, uint8_t data_len, uint8_t* const free_index);
static bool dbal_save_msg_to_repeat(struct dbal_instance* const inst, enum DBAL_MessageType dbal_type, uint16_t service_id, uint16_t command_id, const uint8_t* const bytes, uint8_t data_len, uint8_t free_index);
static void dbal_prepare_header_and_seq_id(struct dbal_instance* const inst, uint8_t repetition);
static bool dbal_append_dbal_frame_to_req_resp_frame(struct dbal_instance* const inst, uint8_t from, enum DBAL_MessageType dbal_type, uint16_t service_id, uint16_t command_id, const uint8_t* const bytes, uint8_t data_len, uint8_t repetition);
static bool dbal_io_dbus_handler_send(struct dbal_instance* const inst, enum DBAL_MessageType dbal_type, uint16_t service_id, uint16_t command_id, const uint8_t* const bytes, uint8_t data_len, uint8_t repetition);
static uint8_t dbal_get_last_sending_status(const struct dbal_instance* const inst, uint8_t msg_index);
static void dbal_con_msg_tx_post_repeat_trigger(struct dbal_instance* const inst);
static void dbal_req_resp_tx_post_action(const struct dbal_instance* const inst);
static bool dbal_check_for_next_msgs_to_send_and_trigger(const struct dbal_instance* const inst, uint8_t tx_index);
static void dbal_prepare_tx_entry(const struct dbal_instance* const inst, uint8_t tx_index, uint8_t data_len);
static uint8_t dbal_get_tx_index(const struct dbal_instance* const inst, uint8_t frame_type); // Simplified frame_type for now
static void dbal_send_connection_message(struct dbal_instance* const inst, enum DBAL_ConnectionMessageType con_message_type);
static void dbal_look_for_msg_reception(struct dbal_instance* const inst, const uint8_t* const bytes, uint8_t data_len);
static void dbal_look_for_ack_msg_reception(struct dbal_instance* const inst, const uint8_t* const bytes, uint8_t data_len);
static bool dbal_is_received_req_resp_msg_corrupt(const struct dbal_instance* const inst, const uint8_t* const bytes, uint8_t data_len);
static bool dbal_is_received_req_resp_msg_to_be_ignored(const struct dbal_instance* const inst, const uint8_t* const bytes);
static void dbal_handle_con_msg(struct dbal_instance* const inst, uint8_t con_msg_type);
...
static void dbal_set_code_section_flag(struct dbal_instance* const inst, uint8_t mask) { inst->CodeSectionBitMask |= mask; }
static bool dbal_check_task_code_section_flag(const struct dbal_instance* const inst) { return (inst->CodeSectionBitMask & 0x01) != 0; } // Assuming 0x01 is task flag
static void dbal_execute_set_non_task_code_sections(struct dbal_instance* const inst) { inst->CodeSectionBitMask = 0; }

static void dbal_clear_io_transmit_buffer(struct dbal_instance* const inst) { inst->DbalFrames2TransmitCnt = 0; inst->TransmitDataLen = 0; memset(inst->TransmitBuffer, 0, sizeof(inst->TransmitBuffer)); }
static void dbal_clear_io_tx_repeat_buffer(struct dbal_instance* const inst) { /* No repeat buffer in this simplified version yet */ } // Original had TxRepeat struct
static void dbal_clear_both_io_tx_buffers(struct dbal_instance* const inst) { dbal_clear_io_transmit_buffer(inst); dbal_clear_io_tx_repeat_buffer(inst); }
static bool dbal_is_io_transmit_buffer_empty(const struct dbal_instance* const inst) { return inst->TransmitDataLen == 0; }
static bool dbal_is_io_tx_repeat_buffer_empty(const struct dbal_instance* const inst) { return true; } // No repeat buffer yet
...
static void dbal_clear_retry_counters(struct dbal_instance* const inst) { memset(inst->SendRetryCounter, 0, sizeof(inst->SendRetryCounter)); }
...
static void dbal_prepare_generic_frame_header(uint8_t* const buffer) {}
static void dbal_store_payload_to_transmit_buffer(uint8_t* const buffer, uint8_t from, const uint8_t* const bytes, uint8_t data_len) {}
static bool dbal_is_msg_to_repeat_saved(const struct dbal_instance* const inst, enum DBAL_MessageType dbal_type, uint16_t service_id, uint16_t command_id, const uint8_t* const bytes, uint8_t data_len, uint8_t* const free_index) { return false; }
static bool dbal_save_msg_to_repeat(struct dbal_instance* const inst, enum DBAL_MessageType dbal_type, uint16_t service_id, uint16_t command_id, const uint8_t* const bytes, uint8_t data_len, uint8_t free_index) { return false; }
static void dbal_prepare_header_and_seq_id(struct dbal_instance* const inst, uint8_t repetition) {}
static bool dbal_append_dbal_frame_to_req_resp_frame(struct dbal_instance* const inst, uint8_t from, enum DBAL_MessageType dbal_type, uint16_t service_id, uint16_t command_id, const uint8_t* const bytes, uint8_t data_len, uint8_t repetition) { return false; }
static bool dbal_io_dbus_handler_send(struct dbal_instance* const inst, enum DBAL_MessageType dbal_type, uint16_t service_id, uint16_t command_id, const uint8_t* const bytes, uint8_t data_len, uint8_t repetition)
{
    bool ret_val = false;
...
static uint8_t dbal_get_last_sending_status(const struct dbal_instance* const inst, uint8_t msg_index) { return 0; }
static void dbal_con_msg_tx_post_repeat_trigger(struct dbal_instance* const inst) {}
static void dbal_req_resp_tx_post_action(const struct dbal_instance* const inst) {}
static bool dbal_check_for_next_msgs_to_send_and_trigger(const struct dbal_instance* const inst, uint8_t tx_index) { return false; }
static void dbal_prepare_tx_entry(const struct dbal_instance* const inst, uint8_t tx_index, uint8_t data_len) {}
static uint8_t dbal_get_tx_index(const struct dbal_instance* const inst, uint8_t frame_type) { return 0; }
static void dbal_send_connection_message(struct dbal_instance* const inst, enum DBAL_ConnectionMessageType con_message_type) {}
static void dbal_look_for_msg_reception(struct dbal_instance* const inst, const uint8_t* const bytes, uint8_t data_len) {}
static void dbal_look_for_ack_msg_reception(struct dbal_instance* const inst, const uint8_t* const bytes, uint8_t data_len) {}
static bool dbal_is_received_req_resp_msg_corrupt(const struct dbal_instance* const inst, const uint8_t* const bytes, uint8_t data_len) { return false; }
static bool dbal_is_received_req_resp_msg_to_be_ignored(const struct dbal_instance* const inst, const uint8_t* const bytes) { return false; }
static void dbal_handle_con_msg(struct dbal_instance* const inst, uint8_t con_msg_type) {}
static bool dbal_call_service_callback(const struct dbal_instance* const inst, enum DBAL_MessageType dbal_type, uint16_t service_id, uint16_t command_id, const uint8_t* const bytes, uint8_t dbal_payload_len) { return false; }