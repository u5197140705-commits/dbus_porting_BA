...
/******************************************************************************/
/*
    This is a GCC scatter file template for platform EFR32xG21.
...
/* 
   ...
   EXAMPLE SW_version part 1:
   
#ifdef APP_SWVERSION_ADDR
  SWVERSION (rx): ORIGIN = APP_SWVERSION_ADDR, LENGTH = APP_SWVERSION_SIZE
#endif
*/

/* When the NVM3 library is used, the calculation of its size must be done in
    <variant>.mak file. The example of definition:
    NVM3_DEFAULT_NVM_SIZE := $(call calcx, 0x3 * 0x$(PAGE_SIZE))
    NVM3_DEFAULT_NVM_END     = $(call calcx,0x$(FWU_OTP_MEMORY_START_ADDRESS)-1)
    NVM3_DEFAULT_NVM_START = $(call calcx,0x$(NVM3_DEFAULT_NVM_END)-0x$(NVM3_DEFAULT_NVM_SIZE)+1)
    defines += NVM3_DEFAULT_NVM_SIZE=UINT32_C(0x$(NVM3_DEFAULT_NVM_SIZE))
    defines += NVM3_DEFAULT_NVM_START=UINT32_C(0x$(NVM3_DEFAULT_NVM_START))

    Do not forget to define new end address of application:
    ROM1_END = $(call calcx,0x$(ROM1_PHY_END) - 0x$(NVM3_DEFAULT_NVM_SIZE) - 0x$(FWU_OTP_MEMORY_SIZE))
 */
#ifdef NVM3_DEFAULT_NVM_START
  NVM3_DEFAULT_NVM (rx): ORIGIN = NVM3_DEFAULT_NVM_START, LENGTH = NVM3_DEFAULT_NVM_SIZE
#endif
