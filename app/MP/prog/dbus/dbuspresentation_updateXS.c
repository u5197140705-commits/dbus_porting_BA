...
uint16 DBPL_ulGetUpdateTransitionDelay(void)
{
    return (uint16)UPDATE_TRANSITION_DELAY_10MS;
}

bool DBPL_bIsRequestedBaudValid(uint16 baud)
{
    uchar i;
    bool ret=false;
    uchar numOfElements=((uchar)sizeof(baud_lookup_table)/(uchar)sizeof(enum DBPL_BaudRate));
    for(i=0;i<numOfElements;i++)
    {   /*lint -e{662} no out of bound access here! */
        if(baud==(uint16)baud_lookup_table[i])
        {
            ret=true;
            break;
        }
...
uint16 DBPL_ulGetBaudrateTransitionDelay(void)
{
    return (uint16)BAUDRATE_TRANSITION_DELAY_10MS;
}

void DBPL_vConfigureBaudrate(uint16 baud)
{
...
uint16 DBPL_uGetUsBitTimeForBaudRate(uint16 baud)
{
    uchar numOfElements=((uchar)sizeof(bit_time_lookup_table)/(uchar)sizeof(enum DBPL_BitTimingUs));
    for(uchar i=0;i<numOfElements;i++)
    {
        if(baud==(uint16)baud_lookup_table[i])
        {
            return (uint16)bit_time_lookup_table[i];
        }
    }
    return 0U;
}

void DBPL_vSetBaudRateTimer(uint16 baudRateTime)
{
    DBPL_tBaudRateTimer=baudRateTime;
}

uint16 DBPL_uiGetBaudRateTimer(void)
{
    return (uint16)DBPL_tBaudRateTimer;
}
...
void DBPL_vSetResetTriggerTimer(uchar resetTriggerTime)
{
...
#if (defined(DBUS2_UPDATE)||defined(DBUS2_UPDATE_HSI))&&defined(FWU_INCLUDED)
struct DBPL_EcuConfigReadResult DBPL_tGetEcuConfigReadResult(uchar objIdNumber)
{   //lint -e{785} "partially initialized struct MisraC2012 9.3 required" other 2 elements initialized later
    ...
    const uint8_t EcuConfigCount = 1U; /*Standard: Programmer NOT updateable AND only one user application.*/
    /*TODO, if programmer updateable: const uint8_t EcuConfigCount = 2U;*/
#ifdef VARIANT_PROGRAMMER
