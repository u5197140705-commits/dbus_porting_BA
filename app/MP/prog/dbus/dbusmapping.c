 ...
 *     \li DBM_UART_vInit(uchar ucIndex)
 ...
 *     \li DBM_UART_vSetTxData(uchar Data)
...
/** Compiler switch. Definition needed for mapping D-Bus-2 mapping layer to a specific UART_CHANNEL. This defined D-Bus-2 for usage of the first UART channel.
...
/* Please make sure the error definition <Terror> is defined for all derivatives, which do not use uchar for this purpose */
...
typedef uchar Terror;
#else
/** Derivative specific error definition (different from uchar) */
...
static uchar DBM_UART_ucTxDisabled = 0U;
...
*/
static uchar DBM_UART_ucRxDisabled = 0;
...
static uint32 DBM_UART_ulNoiseCounter = 0U; //!< Counter, which is incremented when corresponding error flag is set by the UART.
static uint32 DBM_UART_ulFramingErrorCounter = 0U; //!< Counter, which is incremented when corresponding error flag is set by the UART.
static uchar DBM_UART_ucOverrunErrorCounter = 0U; //!< Counter, which is incremented when corresponding error flag is set by the UART.
static uint32 DBM_UART_ulBreakCounter = 0U; //!< Counter, which is incremented when corresponding error flag is set by the UART.
#endif

/************************************************************************/
void DBM_UART_vInit(uchar ucIndex)
{
   /* HUART0_vInit(ucIndex) when UART_CHANNEL is 0 */
   DBM_MAP_UART(UART_CHANNEL, _vInit)(ucIndex);
...
{
   /* HUART0_vEnable() when UART_CHANNEL is 0 */
   DBM_UART_DISABLE_INT();
   DBM_MAP_UART(UART_CHANNEL, _vEnable)();
   DBM_UART_ENABLE_INT();
}
/************************************************************************/
void DBM_UART_vDisableTxEvent(void)
{
   ...
   /* HUART0_vDisableTxEvent(); when UART_CHANNEL is 0 */
   DBM_UART_DISABLE_INT();
   DBM_MAP_UART(UART_CHANNEL, _vDisableTxEvent)();
   DBM_UART_ENABLE_INT();
#else
...
{
#ifndef __DBM_DBUS_MAPPING_LOCK_TX_DISABLE
   /* HUART0_vEnableTxEvent(); when UART_CHANNEL is 0 */
   DBM_UART_DISABLE_INT();
   DBM_MAP_UART(UART_CHANNEL, _vEnableTxEvent)();
#ifdef PSoC4
   DBM_MAP_UART(UART_CHANNEL, _vSetTxEvent)();
#endif
#ifdef __DBM_INITIAL_MANUAL_TX_INT_CALL_NEEDED
   //lint -e{9029} some platforms uses historic BOOL others bool, tested for both
   if (DBM_MAP_UART(UART_CHANNEL, _bIsTxEvent)() == 0U) //if HUART0_bIsTxEvent()
   {
   ...
   {
      DBM_MAP_UART(UART_CHANNEL, _vEnableTxEvent)(); //Enable TxEvent
...
#ifdef __DBM_INITIAL_MANUAL_TX_INT_CALL_NEEDED
   //lint -e{9029} some platforms uses historic BOOL others bool, tested for both
   if (DBM_MAP_UART(UART_CHANNEL, _bIsTxEvent)() == 0U)
   {
...
void DBM_UART_vDisableRxEvent(void)
{
   ...
   /* HUART0_vDisableRxEvent(); when UART_CHANNEL is 0 */
   DBM_UART_DISABLE_INT();
   DBM_MAP_UART(UART_CHANNEL, _vDisableRxEvent)();
   DBM_UART_ENABLE_INT();
#else
   //__DBM_DBUS_MAPPING_LOCK_RX_DISABLE
...
{
#ifndef __DBM_DBUS_MAPPING_LOCK_RX_DISABLE
   /* HUART0_vEnableRxEvent(); when UART_CHANNEL is 0 */
   DBM_UART_DISABLE_INT();
   DBM_MAP_UART(UART_CHANNEL, _vEnableRxEvent)();
   DBM_UART_ENABLE_INT();
#else
   /* Enable RxEvent only once, as this will never be disabled again under this compiler switch setting. */
   static uchar DBM_UART_bRxEventAlreadyEnabled;
   DBM_UART_DISABLE_INT();
...
{
   /* return HUART0_ucGetRxError(); when UART_CHANNEL is 0 */
      Terror tError;
#ifdef __DBM_DBUS_MAPPING_LOCK_RX_DISABLE
   DBM_UART_DISABLE_INT();
#endif
   tError = (Terror)DBM_MAP_UART(UART_CHANNEL, _ucGetRxError)();
...
/************************************************************************/
uchar DBM_UART_ucGetStatus(void)
{
   uchar ucStatus;
   DBM_UART_DISABLE_INT();
   ucStatus = (uchar)DBM_MAP_UART(UART_CHANNEL, _ucGetStatus)();
   DBM_UART_ENABLE_INT();
   return ucStatus;
   /* return HUART0_ucGetStatus(); when UART_CHANNEL is 0  */
}
/************************************************************************/
uchar DBM_UART_ucGetRxData(void)
{
   /* return HUART0_ucGetRxData(); when UART_CHANNEL is 0 */
   uchar ucData;
#ifdef __DBM_DBUS_MAPPING_LOCK_RX_DISABLE
   if (DBM_UART_ucRxDisabled != DBM_UART_RX_EVENT_DURING_LOCK_STATE)
   {
      DBM_UART_DISABLE_INT();
      ucData = DBM_MAP_UART(UART_CHANNEL, _ucGetRxData)();
      DBM_UART_ENABLE_INT();
   }
   else
   {
...
void DBM_UART_vSetTxData(uchar ucData)
{
   /* HUART0_vSetTxData(Data); when UART_CHANNEL is 0 */
#ifdef __DBM_DBUS_MAPPING_LOCK_TX_DISABLE
   DBM_UART_DISABLE_INT();
#endif
   DBM_MAP_UART(UART_CHANNEL, _vSetTxData)(ucData);
#ifdef __DBM_DBUS_MAPPING_LOCK_TX_DISABLE
   DBM_UART_ENABLE_INT();
#endif
}
...
#else
   return DBM_MAP_UART(UART_CHANNEL, _bIsRxOnGoing)() ? TRUE : FALSE;
#endif
}
/************************************************************************/
void DBM_UART_vHandleRxEvent(void)
{
   ...
   if (DBM_UART_ucRxDisabled != 0U)
   {
      ...
      DBM_UART_ucRxError    = DBM_MAP_UART(UART_CHANNEL, _ucGetRxError)();
      DBM_UART_ucRxData     = DBM_MAP_UART(UART_CHANNEL, _ucGetRxData)();
   }
   else
   {
      DLL_vHandleRxEvent();
   ...
   ucError = (uchar)DBM_MAP_UART(UART_CHANNEL, _ucGetRxError)(); // dummy read to make sure Rx Interrupts will be generated even if an overrun error occured between reading of U0RBH and U0RBL (which would cause DBus to stop receiving any data otherwise because of missing Rx interrupts)
   (void)ucError;
#endif
}
/************************************************************************/
/* void HUART0_vHandleTxEvent(void) when UART_CHANNEL is 0 */
...
/************************************************************************/
void DBM_UART_vSetBaudRate(uchar ucIndex, uint16 uiBaudRate)
{
    (void)DBM_MAP_UART(UART_CHANNEL, _bConfigure)(ucIndex,uiBaudRate,HUART_PARITY_NONE,HUART_STOP_BITS_1);
...
/* Statistics */
#ifndef __DBM_EXCLUDE_ERROR_COUNTERS
uint32 DBM_UART_ulGetNoiseCounter(void)
{
   return DBM_UART_ulNoiseCounter;
}
uchar DBM_UART_ucGetUartOverrunCounter(void)
{
   return DBM_UART_ucOverrunErrorCounter;
}
uint32 DBM_UART_ulGetUartFramingErrorCounter(void)
{
   return DBM_UART_ulFramingErrorCounter;
}
uint32 DBM_UART_ulGetUartBreakCounter(void)
{
...
/* Timer functions */
uint32 DBM_ulGetCircleMicroSeconds(void)
{
