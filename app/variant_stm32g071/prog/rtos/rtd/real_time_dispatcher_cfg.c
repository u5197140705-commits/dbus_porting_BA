...
struct RTD_ProjectInventory RTD_DefaultProjectCatalogue =
{
    ...
    ***************************************************************************************************/
    .RTD_TasksList =
    {
        [MOD_BAL]        = {MOD_BAL,       THREAD_BLUEBIRD,         RTD_DEFAULT_INTERVAL,   RTD_INTERVAL_COUNTER_INIT},
        [MOD_DBPL]       = {MOD_DBPL,      THREAD_BLUEBIRD,         RTD_DEFAULT_INTERVAL,   RTD_INTERVAL_COUNTER_INIT},
        [MOD_DLL]        = {MOD_DLL,       THREAD_BLUEBIRD,         RTD_DEFAULT_INTERVAL,   RTD_INTERVAL_COUNTER_INIT},
 ...
 *         header file to use below Formats.
 *
 * * \detail  To define a queue with the name EXA_msgQueue a message size of 8 bytes and a maximum number
 *            of 10 messages use:
 *              RTOS_DEFINE_MSG_QUEUE(EXA_msgQueue, 8, 10);
 *            Note:
...
extern RTOS_MSG_QUEUE EXA_msgQueue;
RTOS_DEFINE_MSG_QUEUE(EXA_msgQueue, 4U, 1U);
 ...
 *
 * * \detail  To define a reference queue with the name Example_refQueue with a maximum of size of 10
 *            references (that means 10 references can be put into the queue before at least one
 *            reference is removed) use the macro:
 *              RTOS_DEFINE_REF_QUEUE(Example_refQueue, 10U);
 *            to use it in another module declare it as:
 *              extern RTOS_REF_QUEUE Example_refQueue;
 *
 * *************************************************************************************************
...
RTOS_DEFINE_REF_QUEUE(EXA_refQueue, 1U);
...
/***************************************************************************************************
 * EVENT FLAG GROUP (FLAG_GRP)
 * *************************************************************************************************
 *  \brief Create Application EVENT FLAG GROUP below.
 *         To create EVENT FLAG GROUP somewhere else in the application, include 'real_time_dispatcher_types.h'
 *         header file to use below Formats.
 *
 *         Examples/Templates for EVENT FLAG GROUP Creation and Initialization:-
 *
 *         1. RTOS_EVENT_FLAG_GROUP eventflag1 =
 *            {
 *                .event_flag_name = "event_flag_example",   --> Give a EVENT FLAG GROUP Human Readable name.
 *            };
 *
 *         2. RTOS_EVENT_FLAG_GROUP eventflag1, eventflag2;  --> In this case, EVENT FLAG GROUP is not assigned
 *                                                               a human readable name
 * *************************************************************************************************
 * \note   1. Register the RTOS_EVENT_FLAG_GROUP below in RTD_EventFlagList[].
 ***************************************************************************************************/
/*! Create Event Flags here */
static char efg_name_dummy[8] = "Example";
static RTOS_EVENT_FLAG_GROUP EFG_DUMMY =
{
    .event_flag_name = efg_name_dummy
}; /*lint !e785 too few initializers - RTOS_EVENT_FLAG_GROUP struct is initialized by RTOS */
/*! Import Event Flags created in other modules here */
#ifdef BLUETOOTH_MODULE
/*! Create or import bluetooth module Event Flags here */
    extern RTOS_EVENT_FLAG_GROUP EDS_EFG_BLE_HIGH;
    extern RTOS_EVENT_FLAG_GROUP EDS_EFG_BLE_MEDIUM;
    extern RTOS_EVENT_FLAG_GROUP EDS_EFG_BLE_LOW;
    extern RTOS_EVENT_FLAG_GROUP EDS_EFG_BLE_LL;
 ...
 *          RTOS_DEFINE_MEMORY_POOL(Example_Pool, sizeof(struct Example), 10U);
 *          to use it in another module declare it as:
...
RTOS_DEFINE_MEMORY_POOL(EXA_pool, sizeof(uint8_t), 1U);
