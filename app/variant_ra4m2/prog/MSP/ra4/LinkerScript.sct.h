...
/******************************************************************************/
/*
    This is a Keil-Arm scatter file template for platform Renesas RA4.
    It will be processed by a C-Preprocessor before interpreted by the linker
 */
/* OPTION-SETTING MEMORY AREA will be flashed by patching together 
 * with option_bytes.eep file
 */
...
// Special regions for data flash
LR_DATA_FLASH DATA_FLASH_START DATA_FLASH_SIZE
{
    ER_DATA_FLASH +0
    {
        *(DATA_FLASH)
...
#ifdef need_ModuleHeader
//TODO: check&correct the placement after enabling FWU
LR_VECTORS ROM1_START + VTOR_ALIGN  ROM1_SIZE - VTOR_ALIGN
#else
LR_VECTORS ROM1_START ROM1_SIZE
#endif
{
...
// original LR_ROM split to two LR_VECTORS and LR_ROM to achieve reservation space for vectors
#ifdef need_ModuleHeader
//TODO: check&correct the placement after enabling FWU
LR_ROM +0  ROM1_SIZE - VTOR_ALIGN - VECTOR_TABLE_SIZE
{
    ER_ROM +0  ROM1_SIZE - VTOR_ALIGN - VECTOR_TABLE_SIZE
#else
LR_ROM  ROM1_START + VECTOR_TABLE_SIZE  ROM1_SIZE - VECTOR_TABLE_SIZE
{
    ER_ROM  ROM1_START + VECTOR_TABLE_SIZE  ROM1_SIZE - VECTOR_TABLE_SIZE
#endif
    {
...
#endif
{
    /* Region ER_FWU_SHARED_DATA.. / ER_BL_FAST_BOOT on fixed memory border
     * It shall be placed at the same address in any linkage
     */
