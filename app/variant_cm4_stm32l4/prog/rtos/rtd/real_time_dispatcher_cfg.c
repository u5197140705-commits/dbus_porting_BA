...
const RTD_ICSHandlers RTD_ISRFunctionsList[] =
{
    ...
    RTDTM_ErrorSystemOverloadedHandler, /* FLAG_0 */  //< Example! Please replace it with user ISR Handler
 ...
 *                .priority_inheritance = false,   --> If priority inheritance needs to be turned on,
 *                                                     set 'true', else 'false'.
 *            };
 *
 *         2. RTOS_MUTEX mutex1, mutex2;           --> In this case, MUTEX is not assigned
 *                                                     a human readable name and by default,
 *                                                     priority inheritance is turned off.
 * *************************************************************************************************
 ...
 *           &mutex_example2, ...
 *
 *        2. To use the Mutex in Application, use following APIs:-
 *           uint8_t RTOS_EnterCriticalSection(RTOS_MUTEX* mutex); --> Get Mutex
 *           uint8_t RTOS_ExitCriticalSection(RTOS_MUTEX* mutex);  --> Release Mutex
 **************************************************************************************************/
