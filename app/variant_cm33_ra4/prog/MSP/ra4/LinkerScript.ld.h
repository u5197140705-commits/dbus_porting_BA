...
/******************************************************************************/
/*
    This is a GCC scatter file template for platform Renesas RA4.
...
#else // RELOC_ADDR = 0 --> ROOT_REGION = ROM1_START
    .text :
    {
        __ROM_Start = .;
        KEEP(*(.vectorsFixed))
...
#else
    __StackTop = ORIGIN(RAM1) + LENGTH(RAM1) - 4;
#endif
