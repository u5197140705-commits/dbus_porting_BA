 ...
 *            - Supported external oscillators are: 10, 16, 20, 24, 25 and 32 MHz.
...
/**************************************************************************************/
/*   Supported CORE_CLOCK frequencies supplied by PLL, based on external oscillator   */
/*------------------------------------------------------------------------------------*/
...
/*     24 MHz  | 12 MHz   | 96  - 240 MHz | 120, 180, 240 MHz                         */
/*     25 MHz  | 12.5 MHz | 100 - 225 MHz | 100, 150, 200 MHz                         */
/**************************************************************************************/

/**************************************************************************************/
/*   Supported CORE_CLOCK frequencies supplied by FLL, based on external oscillator   */
/*------------------------------------------------------------------------------------*/
...
#define RANGE_LOW               (0U)    ///< External clock range 32 to 39 kHz
#define RANGE_HIGH              (1U)    ///< External clock range 3 to 8 MHz
#define RANGE_VERY_HIGH         (2U)    ///< External clock range 8 to 32 MHz
 ...
 *            External oscillators with frequency 10, 16, 20, 24, 25 and 32 MHz are supported
 */
...
#if (EXT_CRYSTAL_OSC_HZ >= MHZ(8U)) && (EXT_CRYSTAL_OSC_HZ <= MHZ(32U))
    #define EXT_OSC_RANGE       RANGE_VERY_HIGH
#elif EXT_CRYSTAL_OSC_HZ >= MHZ(3U)
...
void BASIC_ConfigureExternalOsc(void)
{
    ...
    MCG_C2 |= (uint8_t)(MCG_C2_EREFS_MASK | ( MCG_C2_RANGE_MASK & (EXT_OSC_RANGE << MCG_C2_RANGE_POS) ) );
