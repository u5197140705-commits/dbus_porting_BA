...
#include "dbus_adapter\DBus_extend_adapter.h"
#include "SubSystem0_implementation\SubSys0_example.h"
#include "DBus_second_user_interface\DBus_second_user_interface.h"
...
const TbusObjectTable BAL_tBusObject[] = {
 ...
 {BAL_SERVICE_SUBSYSTEM_ID,
    REC_SERVICE_MSG_TABLE,
    ptDBExSubSys0TransmitTable,
    &DBEx_SubSys0_NumberOfElementsInSubsystem,
    &DBEx_SubSys0_TransmitFlagSubsystem[0]}, /*lint !e929 Cast tested to work correctly */
    
 /* DBusExample user message subsystem */
 {LED_CONTROL_SUBSYSTEM_ID,
    ptDBExReceiveTable,
    ptDBExTransmitTable,
    &DBEx_NumberOfElementsInSubsystem,
    &DBEx_TransmitFlagSubsystem[0]} /*lint !e929 Cast tested to work correctly */
...
{
   static uint8_t BAL_aucErrorCounter[BAL_ERROR_TYPES];
...
BOOL BAL_bIsIdentifierInRangeOfReceiveTable(TbusMessageIdentifier tMessageIdentifier, uint8_t ucSubsystemIndex)
{
    ...
    uint8_t subsystemIndexDADA = BAL_ucGetSubsystemIndex(LED_CONTROL_ADAPTER_SUBSYSTEM_ID);
    uint16_t lowestMsgIdInDADALib = (uint16_t)DADA_MSG_RANGE_START;
    uint16_t highestMsgIdInDADALib = (uint16_t)DADA_MSG_RANGE_END;
    if(subsystemIndexDADA == ucSubsystemIndex)
    {
         if((tMessageIdentifier >= lowestMsgIdInDADALib) && (tMessageIdentifier <= highestMsgIdInDADALib))
         {
             ret=TRUE;
         }
    }
