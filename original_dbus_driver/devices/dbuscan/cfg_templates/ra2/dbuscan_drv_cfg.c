...
{
    .mspi = &MSPI0,  // Important note: When SPI unit is changed, don't forget to change also corresponding interrupt settings in IntTbl module! Follow example of default settings there.
    .sclk = &MDIO102_MSPI0_RSPCKA,
    .miso = &MDIO100_MSPI0_MISOA,
    .mosi = &MDIO101_MSPI0_MOSIA,
...
const struct MDIO_Channel* DBCDRV_csPin = &MDIO103;
#endif
...
{
    return &MEXTI104; // Important note: When mapping to different pin, don't forget to change also corresponding interrupt settings in IntTbl module! Follow example of default settings there.
}
