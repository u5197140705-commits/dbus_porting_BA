...
/** \file
 *
 *   \brief    MKV5x specific mapping library for D-Bus-2 data link layer using
...
/** Compiler switch. Definition needed for mapping D-Bus-2 mapping layer to a 
    specific DBUS_UART_CHANNEL. This defined D-Bus-2 for usage of the first 
    UART channel.
...
/******************************************************************************/
/* PRIVATE DATA AND AND PRIVATE MACRO FUNCTION DEFINITIONS                    */
/******************************************************************************/

/*lint -e9023 -e9024 -e9026 -e2666 #/## usage, function like macro */
    ...
    #define DBM_BREAK_MDIO_CHANNEL    MDIOA3  // default definition
#endif
...
{
    DBM_INT_FUNC(DBUS_HINT_CHANNEL,_vEnableEvent);
}

/** Disable the used hardware interrupt */
void DBM_INT_DisableIrq(void)
{
    DBM_INT_FUNC(DBUS_HINT_CHANNEL,_vDisableIrq);
}
