 ...
 *
 *   \brief    SH30F specific mapping library for D-Bus-2 data link layer using
...
#else
    #error "DBus IDLE detection unavailable due to missing MCAL MEXTI library"
...
void DBM_GPIO_SetBreakPinModeOutput(void)
{
    ...
    /* NOTE: changing AF setting of a pin will cause other pins in the same port
            to change for short period */
    MDIO_configureAlternateFunction(port, pin, MDIO_AF1);
...
void DBM_GPIO_SetBreakPinModeAlternateFunction(void)
{
    ...
    /* NOTE: changing AF setting of a pin will cause other pins in the same port
            to change for short period */
    MDIO_configureAlternateFunction(port, pin, DBUS_MUART_PORT.txPin->scfg.altFunc);
