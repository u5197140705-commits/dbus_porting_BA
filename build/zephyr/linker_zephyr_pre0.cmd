...
MEMORY
    {
    FLASH (rx) : ORIGIN = (0x18000000 + 0x0), LENGTH = (65536 * 1024 - 0x0 - 0x0)
    RAM (wx) : ORIGIN = 0x10040000, LENGTH = (960 * 1K)
    SMU1 ( rw ) : ORIGIN = (1094189056), LENGTH = (522240) SMU2 ( rw ) : ORIGIN = (1144782848), LENGTH = (143360)
...
.intList :
{
 ...
 KEEP(*(.gnu.attributes))
    ...
    SMU1 (NOLOAD) : { __SMU1_start = .; KEEP(*(SMU1)) KEEP(*(SMU1.*)) __SMU1_end = .; } > SMU1 __SMU1_size = __SMU1_end - __SMU1_start; __SMU1_load_start = LOADADDR(SMU1); SMU2 (NOLOAD) : { __SMU2_start = .; KEEP(*(SMU2)) KEEP(*(SMU2.*)) __SMU2_end = .; } > SMU2 __SMU2_size = __SMU2_end - __SMU2_start; __SMU2_load_start = LOADADDR(SMU2);
.last_section :
{
  KEEP(*(.last_section))
} > FLASH
...
noinit (NOLOAD) :
{
    ...
    __kernel_ram_end = 0x10040000 + (960 * 1K);
